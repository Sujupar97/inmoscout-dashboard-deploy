...
import { fetchComparativeAnalysis, fetchFilteredProperties } from '../services/propertyService';
import { ComparativeAnalysisData } from '../supabase';
...
const PriceDistributionChart: React.FC<{ properties: Property[] }> = ({ properties }) => {
    const chartData = useMemo(() => {
        ...
        const buckets = Array.from({ length: bucketCount }, (_, i) => {
            ...
            return { label: `$${(lowerBound / 1000).toFixed(0)}k-$${(upperBound / 1000).toFixed(0)}k`, count: 0 };
        });
            ...
            {chartData.buckets.map((bucket, i) => (
                <div key={i} className="flex items-center text-xs">
                    ...
                    <div className="flex-1 bg-[var(--bg-primary)] rounded-full h-4">
                        <div className="bg-gradient-to-r from-teal-500 to-green-500 h-4 rounded-full flex items-center justify-end pr-2"
...
export const ComparativeAnalysisView: React.FC<ComparativeAnalysisViewProps> = ({ onSelectProperty }) => {
    ...
    const [analysisData, setAnalysisData] = useState<ZonaStats[]>([]);
    const [top5, setTop5] = useState<Property[]>([]);
    const [segmentProperties, setSegmentProperties] = useState<Property[]>([]);
    const [isSegmentLoading, setIsSegmentLoading] = useState(false);

    // Load analysis data from server on mount
    useEffect(() => {
        setIsLoading(true);
        Promise.all([
            fetchComparativeAnalysis(),
            // Load top 5 opportunities (sorted by discount, limit 5)
            fetchFilteredProperties(
                { location: 'All', portal: 'All', minPrice: '', maxPrice: '', minDiscount: '-100', status: 'All',
                  minBeds: '', maxBeds: '', minPricePerSqm: '', maxPricePerSqm: '', minPricePerTotalSqm: '',
                  maxPricePerTotalSqm: '', minDaysOnMarket: '', maxDaysOnMarket: '', minArea: '', maxArea: '',
                  isPotentialOpportunity: false, showOnlyDiscarded: false, propertyType: 'All', searchUrl: '' },
                'opportunities',
                { key: 'discount', direction: 'asc' },
                0, 5
            ),
        ]).then(([analysis, topOpps]) => {
            if (analysis?.byZoneAndType) {
                // Group by zona
                const zonaMap = new Map<string, ZonaStats>();
                for (const item of analysis.byZoneAndType) {
                    if (!zonaMap.has(item.zona)) {
                        zonaMap.set(item.zona, { zona: item.zona, totalCount: 0, avgSqmPrice: 0, propertyTypes: [] });
                    }
                    const z = zonaMap.get(item.zona)!;
                    z.propertyTypes.push({
                        type: item.propertyType,
                        count: item.count,
                        avgSqmPrice: item.avgPricePerSqm || 0,
                        priceRange: { min: item.minPrice || 0, max: item.maxPrice || 0 },
                    });
                    z.totalCount += item.count;
                }
                // Calculate zone avg
                for (const z of zonaMap.values()) {
                    const totalWeighted = z.propertyTypes.reduce((s, pt) => s + pt.avgSqmPrice * pt.count, 0);
                    z.avgSqmPrice = z.totalCount > 0 ? totalWeighted / z.totalCount : 0;
                    z.propertyTypes.sort((a, b) => b.count - a.count);
                }
                setAnalysisData([...zonaMap.values()].sort((a, b) => b.totalCount - a.totalCount));
            }
            if (topOpps?.data) {
                setTop5(topOpps.data);
            }
        }).finally(() => setIsLoading(false));
    }, []);

    // Load segment properties when user selects a zona+type
    useEffect(() => {
        if (!selectedSegment) { setSegmentProperties([]); return; }
        setIsSegmentLoading(true);
        fetchFilteredProperties(
            { location: selectedSegment.zona, portal: 'All', minPrice: '', maxPrice: '', minDiscount: '', status: 'All',
              minBeds: '', maxBeds: '', minPricePerSqm: '', maxPricePerSqm: '', minPricePerTotalSqm: '',
              maxPricePerTotalSqm: '', minDaysOnMarket: '', maxDaysOnMarket: '', minArea: '', maxArea: '',
              isPotentialOpportunity: false, showOnlyDiscarded: false, propertyType: selectedSegment.propertyType, searchUrl: '' },
            'inventory',
            { key: 'discount', direction: 'asc' },
            0, 50
        ).then(result => {
            setSegmentProperties(result.data);
    ...
    const selectedSegmentStats = useMemo(() => {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="lg:col-span-3 bg-[var(--bg-secondary)] p-4 sm:p-6 rounded-lg border border-[var(--border-primary)] h-[500px] lg:h-auto lg:max-h-[calc(100vh-22rem)] flex flex-col">
                    {selectedSegmentStats && selectedSegment ? (
                        <>
                            <h3 className="text-xl font-bold text-[var(--text-primary)] mb-4 flex-shrink-0">Análisis: <span className="text-[var(--primary-accent-text)]">{selectedSegmentStats.type}s en {selectedSegment.zona}</span></h3>
                            {isSegmentLoading ? (
                                ...
                                <div className="space-y-4 overflow-y-auto">
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        <StatCard icon={HomeIcon} label="Propiedades" value={selectedSegmentStats.count.toLocaleString('es-ar')} />
                                        <StatCard icon={PriceIcon} label="$/m² Promedio" value={`$${selectedSegmentStats.avgSqmPrice.toFixed(0)}`} />
                                        <StatCard icon={ChartBarIcon} label="Rango de Precios" value={`$${(selectedSegmentStats.priceRange.min/1000).toFixed(0)}k - $${(selectedSegmentStats.priceRange.max/1000).toFixed(0)}k`} />
                                    </div>

                                    <div className="pt-2">
                                        <h4 className="text-base font-semibold text-[var(--text-primary)] mb-2">Distribución de Precios</h4>
                                        <PriceDistributionChart properties={segmentProperties} />
                                    </div>

                                    <div className="pt-2">
                                        <h4 className="text-base font-semibold text-[var(--text-primary)] mb-2">Top Oportunidades del Segmento</h4>
                                        <div className="space-y-2">
                                            {segmentProperties.slice(0, 5).map(p => (
                                            ...
                                            {segmentProperties.length === 0 && (
                                                <p className="text-sm text-[var(--text-secondary)]">No hay propiedades en este segmento.</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
